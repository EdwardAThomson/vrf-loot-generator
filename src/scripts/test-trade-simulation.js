...
async function step5_BothPlayersCommit() {
  ...
  return new Promise((resolve) => {
    ...
    player1Socket.on('trade:committed', (tradeId, playerId) => {
      log(PLAYER1_NAME, `Received commit notification from ${playerId === player1Id ? 'self' : PLAYER2_NAME}`);
      if (playerId === player2Id) player2Committed = true;
      if (player1Committed && player2Committed) resolve();
    });

    player2Socket.on('trade:committed', (tradeId, playerId) => {
      log(PLAYER2_NAME, `Received commit notification from ${playerId === player2Id ? 'self' : PLAYER1_NAME}`);
      if (playerId === player1Id) player1Committed = true;
      if (player1Committed && player2Committed) resolve();
    });
...
async function step6_BothPlayersReveal() {
  ...
  return new Promise((resolve) => {
    ...
    player1Socket.on('trade:revealed', (tradeId, playerId, items) => {
      const playerName = playerId === player1Id ? PLAYER1_NAME : PLAYER2_NAME;
      log(PLAYER1_NAME, `Received reveal from ${playerName} (${items.length} items)`);
      if (playerId === player2Id) player2Revealed = true;
      if (player1Revealed && player2Revealed) resolve();
    });

    player2Socket.on('trade:revealed', (tradeId, playerId, items) => {
      const playerName = playerId === player2Id ? PLAYER2_NAME : PLAYER1_NAME;
      log(PLAYER2_NAME, `Received reveal from ${playerName} (${items.length} items)`);
      if (playerId === player1Id) player1Revealed = true;
      if (player1Revealed && player2Revealed) resolve();
    });
